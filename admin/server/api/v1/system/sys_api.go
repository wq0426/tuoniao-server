...
func (s *SystemApiApi) CreateAgent(c *gin.Context) {
	var agent system.SysAgent
	err := c.ShouldBindJSON(&agent)
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	//err = utils.Verify(agent, utils.ApiVerify)
	//if err != nil {
	//	response.FailWithMessage(err.Error(), c)
	//	return
	//}
	// 从agent.Site中解析出share_id
	agent.ShareId = utils.GetShareId(agent.Site)
	err = apiService.CreateAgent(agent)
	if err != nil {
		global.GVA_LOG.Error("创建失败!", zap.Error(err))
		response.FailWithMessage("创建失败", c)
		return
...
func (s *SystemApiApi) GetAgentList(c *gin.Context) {
	list, err := apiService.GetAgentList()
	if err != nil {
...
func (s *SystemApiApi) GetFreeSite(c *gin.Context) {
	var req request.GetFreeSiteReq
	...
	// validate
	if code := req.Validate(); code > 0 {
		response.Result(code, nil, "请选择日期", c)
		return
	}
	res, err := apiService.GetFreeSite(c, req.RegionId, req.StartCreatedAt, req.EndCreatedAt, req.RoomId, req.BedId)
	if err != nil {
